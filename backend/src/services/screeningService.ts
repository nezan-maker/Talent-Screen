...
import type { ScreeningEvaluation } from "../types/talentProfile.js";
...
function parseRequiredYears(job: any) {
  ...
  if (text.includes("lead")) return 7;
...
function tokenizeJobCriteria(job: any) {
  ...
  return {
    mustHave,
    niceToHave,
    descriptionTokens: splitList(
      [job?.job_description, job?.job_qualifications, job?.job_responsibilities]
        .filter(Boolean)
        .join(", "),
    ),
  };
...
function fuzzyIncludes(values: string[], target: string) {
...
function summarizeEducation(education: ReturnType<typeof normalizeEducation>) {
  ...
  if (
    /computer|software|information|design|human resource|business/i.test(field)
  ) {
    fieldRelevance = "High";
    score = 82;
  } else if (field) {
...
function detectCareerGap(experience: ReturnType<typeof normalizeExperience>) {
  const parsedRanges = experience
    ...
    .filter(
      (item) => !Number.isNaN(item.start) && !Number.isNaN(item.end) && item.end >= item.start,
    )
...
export async function evaluateApplicantsForJob(input: {
  ...
  const evaluated = input.applicants.map((applicant) => {
    ...
    const matched = jobCriteria.mustHave.filter((item) =>
      fuzzyIncludes(evidencePool, item),
    );
    const missing = jobCriteria.mustHave.filter(
      (item) => !fuzzyIncludes(evidencePool, item),
    );
    const niceMatches = jobCriteria.niceToHave.filter((item) =>
      fuzzyIncludes(evidencePool, item),
    );
    const skillsScore = clampScore(
      (jobCriteria.mustHave.length
    ...
    const relevantYears = Math.min(
      totalYears,
      matched.length > 0 ? totalYears : Math.max(0, totalYears - 1),
    );
    const experienceScore = clampScore(
      Math.min(100, (totalYears / Math.max(requiredYears, 1)) * 75 + (matched.length > 0 ? 20 : 10)),
    );
    ...
    const relevantCertifications = (applicant?.certifications ?? [])
      ...
      .filter((name: string) =>
        fuzzyIncludes(
          [...jobCriteria.mustHave, ...jobCriteria.niceToHave, ...jobCriteria.descriptionTokens],
          name,
        ),
      );
    ...
    const languageScore = clampScore(
      languages.some((language) => trimText(language.name).toLowerCase() === "english")
        ? 95
        : languages.length > 0
          ? 75
          : 60,
    ...
    const overallScore = clampScore(
      skillsScore * DEFAULT_WEIGHTS.skills_match +
    ...
    const overqualified =
      requiredYears <= 1 && totalYears >= 6 && overallScore >= 85;
    ...
    return {
      ...
      overall: {
        ...
        verdict: "Review" as "Review" | "Shortlisted" | "Rejected",
        summary,
      },
      dimension_scores: {
        skills_match: {
  ...
  if (enriched) {
    for (const item of enriched) {
      const match = evaluated.find(
        (evaluation) => evaluation.candidate_id === trimText(item.candidate_id),
      );
      if (!match) {
        continue;
      }
  ...
  const results: ScreeningEvaluation[] = evaluated.map((item) => ({
