...
function normalizeApplicantKey(value: string) {
  return value
    .toLowerCase()
    .replace(/[_-]+/g, " ")
    .replace(/\s+/g, " ")
...
function inferApplicantEmail(input: {
  ...
  const additionalEmail = input.additional_info.find((item) =>
    /\S+@\S+\.\S+/.test(item),
  );
...
function normalizeCandidateInput(input: any): NormalizedCandidateInput | null {
  ...
  return {
    ...
    education_certificates: splitList(
      input?.education_certificates ?? input?.education ?? input?.certifications,
    ),
    additional_info: Array.from(new Set(additionalInfo.filter(Boolean))),
    experience_in_years: parseExperience(
      input?.experience_in_years ?? input?.experience ?? input?.years_experience,
    ),
  };
}
...
function cellValue(value: unknown) {
  ...
  if (typeof value === "object" && value !== null && "text" in value) {
    return trimText((value as { text?: unknown }).text);
  }

  return trimText(value);
}

function worksheetToRecords(worksheet: excel.Worksheet) {
...
async function createApplicantsFromInputs(inputs: NormalizedCandidateInput[]) {
...
function buildExampleForm(payload: {
  ...
  return {
    ...
    CORE_STRENGTHS: payload.job_ai_criteria.map((criterion) =>
      criterion.criteria_string.toUpperCase(),
    ),
  };
}
...
function detectRequiredYears(experienceLevel: string) {
...
function matchScore(candidateTerms: string[], desiredTerms: string[]) {
  if (desiredTerms.length === 0) {
    return candidateTerms.length > 0 ? 75 : 40;
  }

  const matches = desiredTerms.filter((desired) =>
    candidateTerms.some(
      (term) => term.includes(desired) || desired.includes(term),
    ),
  );
  return clamp((matches.length / desiredTerms.length) * 100, 0, 100);
}

function scoreApplicant(job: any, applicant: any) {
  const criteriaText = Array.isArray(job.job_ai_criteria)
    ? job.job_ai_criteria
        ...
        .join(", ")
    : "";
  const candidateSkills = splitList(applicant.skills).map((skill) =>
    skill.toLowerCase(),
  );
  const candidateEducation = splitList(applicant.education).map((entry) =>
    entry.toLowerCase(),
  );
  const desiredTerms = splitKeywords(
    `${criteriaText} ${job.job_qualifications ?? ""}`,
  );
  const skillsScore = matchScore(candidateSkills, desiredTerms);
  const educationScore = candidateEducation.length > 0 ? 80 : 45;
  const requiredYears = detectRequiredYears(
    trimText(job.job_experience_required || "mid"),
  );
  ...
  const overall = clamp(
    skillsScore * 0.55 + educationScore * 0.15 + experienceScore * 0.3,
    0,
    100,
  );

  const matchedSkills = desiredTerms.filter((desired) =>
    candidateSkills.some(
      (skill) => skill.includes(desired) || desired.includes(skill),
    ),
  );
...
async function sendShortlistMessage(email: string, jobTitle: string) {
...
export const registerCandidates = async (req: Request, res: Response) => {
  try {
    const requestWithFile = req as Request & { file?: Express.Multer.File };
    let inputs: NormalizedCandidateInput[] = [];
    ...
    if (requestWithFile.file) {
      ...
      inputs = rows
        ...
        .filter(
          (value): value is NormalizedCandidateInput => value !== null,
        );
...
export const uploadResumeZip = async (req: Request, res: Response) => {
  try {
    const requestWithFile = req as Request & { file?: Express.Multer.File };
    ...
    const matchedApplicants: string[] = [];

    for (const entry of entries) {
      const fileName = path.parse(entry.path).name;
      const applicant = await Applicant.findOne({
        applicant_name: new RegExp(
          `^${normalizeApplicantKey(fileName).replace(/\s+/g, "\\s+")}$`,
          "i",
        ),
      });
...
export const createJob = async (req: Request, res: Response) => {
  try {
    const payload = (req.body?.reqBody ?? req.body) as {
      job_ai_criteria?: JobCriterion[];
...
export const reviewResult = async (req: Request, res: Response) => {
  try {
