import type { TalentAvailability } from "../types/talentProfile.js";
import {
...
export type ResumeHeuristicFields = {
...
function similarityScore(left: string, right: string) {
  ...
  const tokenScore =
    overlap === 0 ? 0 : overlap / Math.max(leftTokens.size, rightTokens.size, 1);
...
export function extractCellText(value: unknown): string {
  ...
  if (typeof value === "object") {
    ...
    const richText = Array.isArray(record.richText)
      ? record.richText
      : Array.isArray((record.text as RecordLike | undefined)?.richText)
        ? ((record.text as RecordLike).richText as unknown[])
        : [];

    if (richText.length > 0) {
      return richText
        .map((item) => trimText((item as RecordLike).text))
        .filter(Boolean)
        .join("")
        .trim();
    }
...
export function extractLinksFromText(value: unknown) {
  const text = extractCellText(value);
  if (!text) {
    return { linkedin: "", github: "", portfolio: "", all: [] as string[] };
  }

  const rawLinks = text.match(
    /(?:https?:\/\/[^\s,;|)]+|www\.[^\s,;|)]+|linkedin\.com\/[^\s,;|)]+|github\.com\/[^\s,;|)]+|[a-z0-9-]+(?:\.[a-z0-9-]+)*\.[a-z]{2,6}(?:\/[^\s,;|)]*)?)/gi,
  ) ?? [];
...
export function extractEmailFromText(value: unknown) {
  return normalizeEmail(extractCellText(value));
}

export function pickBestRecordValue(
  record: RecordLike,
  aliases: string[],
...
function sectionIndexes(lines: string[]) {
...
function sectionText(lines: string[], headings: Map<string, number>, name: string) {
...
export function parseResumeHeuristics(text: string): ResumeHeuristicFields {
  ...
  const topLinkedin =
    topTokens.find((token) => token.toLowerCase().includes("linkedin.com")) || "";
  const topGithub =
    topTokens.find((token) => token.toLowerCase().includes("github.com")) || "";
  const topPortfolio =
    topTokens.find(
      (token) =>
        /\.[a-z]{2,6}(?:\/.*)?$/i.test(token) &&
        !token.includes("@") &&
  ...
  return {
    ...
    headline:
      lines[1] && !/summary|experience|skills|education/i.test(lines[1]) ? lines[1] : "",
    bio: summaryText || "",
    email,
