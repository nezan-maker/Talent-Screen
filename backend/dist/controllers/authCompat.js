...
async function issueSignupVerificationChallenge(res, user) {
...
function setGoogleStateCookie(res, value) {
    res.cookie("google_oauth_state", value, getCookieOptions(10 * 60 * 1000));
}
async function exchangeGoogleCodeForUser(req, code) {
...
async function finalizeGoogleAuthentication(res, user) {
    user.isVerified = true;
    user.sign_otp_token = null;
    user.confirmation_link_id = "";
    await user.save();
...
export const googleStart = async (req, res) => {
    try {
        ...
        return res.redirect(`${getFrontendRedirectUrl("/login")}?error=google_oauth_start_failed`);
    }
};
export const googleCallback = async (req, res) => {
    try {
        ...
        if (!code || !state || !cookieState || state !== cookieState) {
            ...
            return res.redirect(`${getFrontendRedirectUrl("/login")}?error=google_oauth_state_invalid`);
        }
        ...
        if (!email || !googleId || !emailVerified) {
            clearSessionCookies(res);
            return res.redirect(`${getFrontendRedirectUrl("/login")}?error=google_oauth_profile_invalid`);
        }
        ...
        const redirectUrl = new URL(getFrontendRedirectUrl(user.onboarding_completed ? "/dashboard" : "/welcome"));
        return res.redirect(redirectUrl.toString());
    }
    catch (error) {
        console.error("Error in googleCallback:", error);
        clearSessionCookies(res);
        return res.redirect(`${getFrontendRedirectUrl("/login")}?error=google_oauth_failed`);
    }
};
...
export const verifyCode = async (req, res) => {
    try {
        ...
        const recoveryReferenceToken = toStringValue(req.cookies?.recovery_reference_token);
