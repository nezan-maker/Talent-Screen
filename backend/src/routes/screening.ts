...
function mapCandidate(applicant: any, index: number): ScreeningCandidateInput {
  ...
  const education = normalizeEducation(applicant?.education).map((item) =>
...
export default function screeningRouter(options: ScreeningRouterOptions = {}) {
  const router = Router();
  ...
  router.post("/run", screeningRunRateLimit, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const legacyResults = ai.shortlist.map((item) =>
        toLegacyScreeningResult({
