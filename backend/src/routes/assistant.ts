...
function mapConversation(conversation: any) {
  return {
    ...
    followUps: Array.isArray(conversation?.follow_ups)
      ? conversation.follow_ups.map((item: unknown) => trimText(item)).filter(Boolean)
      : [],
...
function mapJob(job: any): AssistantJobContext {
  ...
  const requirements = splitList(
    [job?.job_qualifications, job?.job_responsibilities, job?.job_description]
...
function mapCandidate(candidate: any, index: number): AssistantCandidateContext {
...
export default function assistantRouter(options: AssistantRouterOptions = {}) {
  const router = Router();
  ...
  router.post("/ask", async (req: Request, res: Response) => {
    try {
      ...
      conversation.messages.push(userMessage as any, assistantMessage as any);
