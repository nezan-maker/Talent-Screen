...
function mapRow(row) {
    const out = { raw: row };
    const get = (names) => pickBestRecordValue(row, names);
    const fullName = get([
        "full name",
        "name",
        "candidate name",
    ...
    if (resumeText) {
        ...
        if (!out.fullName && heuristic.applicant_name)
            out.fullName = heuristic.applicant_name;
        if (!out.email && heuristic.email)
            out.email = heuristic.email;
        if (!out.location && heuristic.location)
            out.location = heuristic.location;
