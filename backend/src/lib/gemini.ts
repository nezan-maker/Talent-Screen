...
function normalizeMinimumMarks(value: unknown) {
...
export function normalizeScreeningVerdict(value: unknown) {
...
function uniqueModels(models: string[]) {
  return Array.from(
    new Set(models.map((item) => trimText(item)).filter(Boolean)),
  );
}
...
export async function reviewApplicantWithGemini(
  opts: GeminiAuth & {
    model: string;
    job: unknown;
    candidate: unknown;
  ...
  const verdict =
    matchScore >= minimumMarks ? "Shortlisted" : "Rejected";
...
export async function parseResumeObjectFieldsWithGemini(
  ...
  try {
    const text = await generateWithGemini({ ...opts, prompt });
    const parsed = JSON.parse(extractFirstJsonObject(text)) as Record<
      string,
      unknown
    >;
