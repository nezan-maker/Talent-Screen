...
export async function reviewApplicant(req, res) {
    try {
        const resultId = trimText(req.params.resultId ?? req.body?.result_id ?? req.body?.screening_result_id);
        const additionalInfo = trimText(req.body?.additional_info ?? req.body?.additionalInfo);
        const userId = req.currentUserId;
        if (!userId) {
            return res.status(401).json({ expiration_error: "Session expired" });
        }
        ...
        if (!auth) {
            return res.status(400).json({
                data_error: "Gemini is not configured. Configure GOOGLE_API_KEY or Vertex settings first.",
            });
        }
        ...
        const updatedResult = await ScreeningResultModel.findByIdAndUpdate(resultId, {
            overall: {
...
export async function finalizeJobRecruiting(req, res) {
    try {
        ...
        const normalizedDecisions = decisions
            ...
            const resultId = trimText(record.result_id ?? record.resultId);
            const applicantId = trimText(record.applicant_id ?? record.applicantId);
            const verdict = trimText(record.verdict);
            return {
                resultId,
                applicantId,
        ...
        const resultByApplicantId = new Map(results.map((item) => [trimText(item.applicant_id), item]));
        let finalizedCount = 0;
        for (const decision of normalizedDecisions) {
            const result = (decision.resultId ? resultById.get(decision.resultId) : undefined) ??
            ...
            await Promise.all([
                ...
                Applicant.findOneAndUpdate({ _id: applicantId, user_id: userId }, {
